...
export function FinVoiceDashboard({ appConfig }: FinVoiceDashboardProps) {
  ...
  const agentParticipant = remoteParticipants.find(
    (p) => p.kind === 'agent' || p.identity?.includes('agent') || (p.attributes && (p.attributes as any).agent_name)
  ) || remoteParticipants[0];
  const activeAgentName = (agentParticipant?.attributes as any)?.agent_name || 'FinVoice';
  ...
  if (isConnecting) {
    ...
    if (isSpeaking) {
      stateLabel = `${activeAgentName} is speaking...`;
      ...
      stateLabel = `${activeAgentName} is thinking...`;
      stateBg = 'bg-amber-50 text-amber-700 border-amber-200';
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-900 flex flex-col font-sans select-none">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-8 py-3.5 shadow-xs flex items-center justify-between">
        ...
        {/* Right: Language Selector Dropdown & Dashboard Links */}
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Link
              href="/escalations"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition-all duration-200 border border-slate-200"
            ...
            <div className="financial-card rounded-3xl p-6 sm:p-8 flex flex-col items-center text-center space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-1">
                ...
                <p className="text-slate-500 text-xs sm:text-sm font-medium">
                  {activeAgentName === 'Government Scheme Specialist'
                    ? 'Welfare & Government Scheme Specialist'
